...
func _draw() -> void:
	...
	for dy in range(-span, span + 1):
		for dx in range(-span, span + 1):
			...
			var offset := Vector2(float(dx), float(dy)) * scale
			...
			_draw_cell(cell, centre + offset, scale)

	# Player marker, always dead centre, pointing the way they face.
	if not blurred:
...
func _draw_cell(cell: Vector2i, at: Vector2, scale: float) -> void:
	var pos := at
	if blurred:
...
func _draw_player(centre: Vector2, scale: float) -> void:
	...
	var forward := Vector2(float(v.x), float(v.y))
