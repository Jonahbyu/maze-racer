...
#
# The CI workflow in tools/ci/deploy.yml.txt does this automatically, but it
# cannot be installed until the gh token carries the 'workflow' scope. Until
# then this script is the deploy path.
#
...
git clone --depth 1 --branch gh-pages https://github.com/Jonahbyu/maze-racer.git $work 2>$null
if (-not (Test-Path $work)) { throw 'clone of gh-pages failed' }
